...
import torch
#torch.cuda.memory_split(512 * 1024 * 1024)
import logging
import os
import random
import shutil
...
if __name__ == '__main__':
    ...
    model_name = ("output/models/5e5550fe-2192-40c9-80e9-ac0536326e52_fine_tuned_facebook_bart-large-mnli")
...
# Add predictions as a new column
    test_df2['predictions'] = test_preds.predicted_labels
    
    new_folder = 'iter_1' 
    new_folder_path = os.path.join(out_dir, new_folder)
    os.mkdir(new_folder_path)

# Save updated dataframe to a new csv file
    test_df2.to_csv(os.path.join(new_folder_path, "test_with_predictions.csv"), index=False)
    evaluate_classification_performance(test_preds.predicted_labels, test_gold_labels, new_folder_path,
                                        info_dict={
                                            'iteration': 1, #change
...
## Save updated dataframe to a new csv file
#        test_df2.to_csv(os.path.join(out_dir, "iter_number, "test_with_predictions.csv"), index=False)
#        
